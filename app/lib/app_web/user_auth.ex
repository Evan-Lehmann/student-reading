defmodule AppWeb.UserAuth do
  ...
  def on_mount(:require_student, _params, session, socket) do
    ...
    if socket.assigns.current_user && socket.assigns.current_user.type == "student" do
      {:cont, socket}
    else
      socket =
        socket
  ...
  def on_mount(:require_teacher, _params, session, socket) do
    ...
    if socket.assigns.current_user && socket.assigns.current_user.type == "teacher" do
      ...
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:error, "Only teachers can access this page!")
        |> Phoenix.LiveView.redirect(to: ~p"/")
  ...
  def on_mount(:require_teacher_or_student, _params, session, socket) do
    socket = mount_current_user(socket, session)

    if socket.assigns.current_user && ((socket.assigns.current_user.type == "student") || socket.assigns.current_user.type == "teacher") do
      {:cont, socket}
    else
      socket =
        socket
  ...
  def require_teacher(conn, _opts) do
    if conn.assigns[:current_user] && conn.assigns[:current_user].type == "teacher" do
      conn
    else
      conn
  ...
  def require_teacher_or_student(conn, _opts) do
    if conn.assigns[:current_user] && ((conn.assigns[:current_user].type == "student") || conn.assigns[:current_user].type == "teacher") do
      conn
    else
      conn
      |> put_flash(:error, "Must be teacher or student in a class!")
