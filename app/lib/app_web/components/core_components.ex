defmodule AppWeb.CoreComponents do
  ...
  def modal(assigns) do
    ...
    >
      ...
      <div
        ...
        tabindex="0"
      >
        <div class="flex min-h-full mt-16 justify-center">
          <div class="w-50 max-w-3xl">
            <.focus_wrap
              id={"#{@id}-container"}
              phx-window-keydown={JS.exec("data-cancel", to: "##{@id}")}
              phx-key="escape"
              phx-click-away={JS.exec("data-cancel", to: "##{@id}")}
              class="shadow-zinc-700/10 ring-zinc-700/10 relative hidden rounded-2xl bg-white p-2 shadow-lg ring-1 transition"
            >
  ...
  def flash(assigns) do
    ...
    >
      ...
      <p class="text-sm leading-5 mb-1"><%= msg %></p>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="block w-full rounded-md border border-gray-300 bg-white focus:border-zinc-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="table w-[40rem] mt-11 sm:w-full">
        <thead class="text-sm text-left leading-6 text-zinc-500">
          <tr>
            <th :for={col <- @col} scope="col" class="p-0 pr-6 pb-4 font-normal"><%= col[:label] %></th>
